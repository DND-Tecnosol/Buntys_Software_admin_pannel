...
export default function WeboffersData() {
  const offers = useSelector((s) => s.campign.vaucher)
  const dispatchs = useDispatch()

  const status = (status, id) => {
    console.log(`${status} ${id}`)
    appAxios.put(`${apiRoutes.CampignData}vaucherstatus/${id}`, { status: status }).then(e => {
      if (e.data.code == 200) {
        // toast(e.data.msg)
          ...
          <TableBody>
            {offers && offers.map((row) => {
              ...
              <StyledTableRow key={row.id} >
                ...
                <StyledTableCell >{row.vaoucherApplicable}</StyledTableCell>
...
const LinkData=({data})=><Link to={`${routes.voucher}/${data.id}`}>{data.name}</Link> 
